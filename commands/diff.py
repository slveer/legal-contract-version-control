...
def get_data_number(
    ...
    for tag in tag_list:
        parsed_tag = (
            tag if hasattr(tag, 'attrs')
            else BeautifulSoup(tag, "html.parser").find()
        )
